...
func newTestCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Runs `go test ./...` with ORJANDA_ENV=test, routing orjanda/testing.NewTestSite to an ephemeral SQLite DB (TAD §16/§17).",
		Args:  cobra.NoArgs,
...
func runTest(ctx context.Context, runFilter string) error {
	...
	c.Env = append(os.Environ(), "ORJANDA_ENV=test")
	c.Stdin = os.Stdin
	c.Stdout = os.Stdout
	c.Stderr = os.Stderr
	return c.Run()
