...
- [x] The agent creates a leave request end-to-end, including the approval confirmation round trip, against a live LLM provider (OpenAI or Anthropic). Verified live via the `openai_compatible` provider against OpenRouter (free model, PRD §44.4 item 4b): the agent discovered the documents, looked up the employee and leave type, hit the Safety Layer's approval gate, and created a `Draft` LeaveRequest read back from the database with a `via_agent` audit trail — see `orjanda-app-hr-example/agent_leave_request_test.go` (`TestAgentLeaveRequestLiveOpenRouter` + the deterministic `TestAgentLeaveRequestApprovalRoundTrip`).
- [x] The agent is denied access to `Salary` for a role without `hr_manager`, verified both in the tool schema (field absent) and in a direct attempt to read it via `get_employee`. Verified by `TestAgentSalaryDeniedNonHRManager` in `orjanda-app-hr-example/phase12_test.go`: `salary` is absent from every tool schema the employee's identity is sent (TAD §10.3 projection), hr_manager's write tools expose it (positive control), and a live `get_employee` read through the Agent Runtime + Document Engine returns no `salary` key.
- [x] The agent operates on all four Documents using only auto-generated tools — zero Document-specific agent code was written for `Department`, `LeaveType`, or `LeaveRequest` (only the one custom `calculate_leave_balance` tool from PRD §24.3/§37 is hand-written, as the PRD itself specifies). Verified by `TestAgentOperatesAllFourDocuments`: exactly one custom tool exists (`tools.CustomTools()`), the full 24-tool generated surface is visible to hr_manager, and a scripted 10-step discovery→operate flow creates and updates all four Documents through the real Agent Runtime, Safety-Layer approval round trips, and Document Engine.
- [x] The audit log contains a complete, correctly `via_agent`-flagged trail for every operation performed during this validation pass, human and agent alike. Verified by `TestAuditTrailCompleteHumanAndAgent`: ten operations (nine human + one agent) produce exactly ten entries with per-entry docType/action/userID matching, human entries carry no agent attribution, and the agent-created LeaveRequest entry carries the session id, the prompt, and `via_agent=true` (TAD §13).
- [x] The leave-approval workflow enforces role-gated transitions exactly as declared (a `Department Head`/`HR Manager`-only `Approve` transition rejects an `Employee`-role attempt). Verified by the extended `TestLeaveApprovalWorkflow` in `orjanda-app-hr-example/hr_test.go`: an `Employee`-role `Reject` attempt fails with `errors.ErrPermission`, while HR Manager `Reject` and `Approve` both succeed through `workflow.Engine` + `perm.Engine`.
- [x] Performance targets from PRD §33.1 hold under the reference Application with a seeded dataset of at least 1,000 `Employee` records. Verified by `TestPerformanceReferenceApp1000Employees`: paginated Document list over 1,000 records at 1.8ms p95 (target < 100ms), REST CRUD read through the real router at 0.4ms p95 (target < 50ms), and reference-Application Registry compilation at 0.16ms (target < 2s; the 100-Document compile target is covered framework-side by `schema.TestRegistry_CompilePerformance`).

**MVP sign-off:** Phase 12's checklist passing in full **is** the MVP Definition of Done (PRD §44.1's central thesis, validated). No phase after this one is required to ship v1.0 of the MVP. **All ten completion criteria above pass against the running reference Application — the MVP is complete.**
